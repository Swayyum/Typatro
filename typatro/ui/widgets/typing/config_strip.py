...
class RunMode(Switchable):
    setting_name = "run"
    setting_icon = "Run"

    def _toggle(self) -> None:
        current = config_parser.get("game_mode")
        config_parser.set("game_mode", "classic" if current == "run" else "run")

    def refresh_setting(self) -> None:
        self.set_class(config_parser.get("game_mode") == "run", "enabled")

    def render(self) -> RenderableType:
        self.refresh_setting()
        mode = "Run" if config_parser.get("game_mode") == "run" else "Classic"
        return self.setting_icon + " " + mode
